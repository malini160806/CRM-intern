...
const AddInvoiceModal = ({ isOpen, onClose, onInvoiceAdded }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('/api/invoices', submissionData, {
        headers: { Authorization: `Bearer ${user.token}` }
      });

      if (onInvoiceAdded) onInvoiceAdded(response.data);
      onClose();
      // Reset form
      setFormData({
        invoiceNumber: `INV-${Math.floor(100000 + Math.random() * 900000)}`,
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
      ...
      >
        <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/50">
          <div>
            <h3 className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tight">Generate Invoice</h3>
            <p className="text-sm text-slate-500 font-medium">Create a new billing statement for your client</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6 max-h-[80vh] overflow-y-auto custom-scrollbar">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Invoice #</label>
          ...
          <div className="space-y-4 pt-4 border-t border-slate-100 dark:border-slate-800">
            <div className="flex items-center justify-between">
              <h4 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-widest">Line Items</h4>
              <button 
                type="button"
                onClick={addItem}
          ...
          <div className="pt-6 flex gap-3">
            ...
            >
              {loading ? <Loader2 className="w-6 h-6 animate-spin" /> : <span className="font-black uppercase tracking-widest">Create Invoice</span>}
            </button>
          </div>
        </form>
      </motion.div>
